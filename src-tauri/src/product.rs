...
pub fn import_sync_json(c: &mut Connection, json: &str) -> Result<SyncSummary, String> {
    let env: BackupEnvelope =
        serde_json::from_str(json).map_err(|e| format!("Backup inválido: {e}"))?;
    if env.format != "ludex-backup" || env.version != 1 {
        return Err("Formato/versão de backup não suportado".into());
    }
    ...
    let tx = c.transaction().map_err(|e| e.to_string())?;
    if let Some(games) = env.data.get("games").and_then(|v| v.as_array()) {
        for g in games {
            ...
            let exists: i64 = tx
                ...
                .map_err(|e| e.to_string())?;
            if exists == 0 {
                tx.execute("INSERT INTO games(id,title,platform,source,favorite,status,normalized_title,created_at,updated_at) VALUES(?1,?2,?3,?4,?5,?6,?7,COALESCE(?8,CURRENT_TIMESTAMP),COALESCE(?9,CURRENT_TIMESTAMP))",params![id,title,g.get("platform").and_then(|v|v.as_str()).unwrap_or("PC"),g.get("source").and_then(|v|v.as_str()).unwrap_or("sync"),g.get("favorite").and_then(|v|v.as_i64()).unwrap_or(0),g.get("status").and_then(|v|v.as_str()).unwrap_or("Quero jogar"),g.get("normalized_title").and_then(|v|v.as_str()).unwrap_or(&normalize_title(title)),g.get("created_at").and_then(|v|v.as_str()),g.get("updated_at").and_then(|v|v.as_str())]).map_err(|e|e.to_string())?;
                summary.inserted += 1;
            } else {
                summary.skipped += 1;
            }
