...
pub fn settings(connection: &Connection) -> Result<StoreSettingsStatus, String> {
    Ok(StoreSettingsStatus {
        ...
        country: crate::db::get_setting(connection, "store.country")?.unwrap_or_else(|| "BR".into()),
    })
}
...
pub fn catalog(connection: &Connection) -> Result<Vec<StoreCatalogItem>, String> {
    let request = client()?.get("https://store.steampowered.com/api/featuredcategories/")
        ...
        if response.status().is_success() {
            if let Ok(value) = response.json::<Value>() {
                ...
                for category in ["specials", "top_sellers", "new_releases", "coming_soon"] {
                    let Some(array) = value.get(category).and_then(|v| v.get("items")).and_then(Value::as_array) else {
                        continue;
                    };
                    for item in array {
                        let Some(id) = item.get("id").and_then(Value::as_u64) else { continue; };
                        ...
                        let currency = item.get("currency").and_then(Value::as_str).map(str::to_string);
                        let cover = item.get("large_capsule_image")
                        ...
                        items.push(StoreCatalogItem {
                            ...
                            currency,
                            discount_percent: item.get("discount_percent").and_then(Value::as_i64).unwrap_or(0),
                        });
                        if items.len() >= 80 { break; }
                    }
                    if items.len() >= 80 { break; }
                }
                if !items.is_empty() {
...
fn steam_offer(http: &Client, app_id: &str) -> Result<(String, Option<String>, Option<StoreOffer>), String> {
    let response = http
        ...
        .map_err(|e| e.to_string())?;
    if !response.status().is_success() {
        return Ok((format!("Steam App {app_id}"), Some(format!("steam-artwork:{app_id}")), None));
    }
    let value: Value = response.json().map_err(|e| e.to_string())?;
    let data = value.get(app_id).and_then(|v| v.get("data"));
    let title = data.and_then(|v| v.get("name")).and_then(Value::as_str).unwrap_or("").to_string();
    let cover = data.and_then(|v| v.get("header_image")).and_then(Value::as_str).map(str::to_string);
    let free = data.and_then(|v| v.get("is_free")).and_then(Value::as_bool).unwrap_or(false);
    if free {
        return Ok((title, cover, Some(StoreOffer {
            shop: "Steam".into(), kind: "official".into(), price: 0.0, regular: Some(0.0), currency: "BRL".into(), cut: 0,
    ...
    let Some(final_price) = final_price else { return Ok((title, cover, None)); };
    Ok((title, cover, Some(StoreOffer {
        shop: "Steam".into(),
        kind: "official".into(),
        price: final_price as f64 / 100.0,
        regular: regular.map(|v| v as f64 / 100.0),
        currency: price.and_then(|v| v.get("currency")).and_then(Value::as_str).unwrap_or("BRL").to_string(),
        cut: price.and_then(|v| v.get("discount_percent")).and_then(Value::as_i64).unwrap_or(0),
        url: format!("https://store.steampowered.com/app/{app_id}/"),
    })))
}

fn itad_offers(connection: &Connection, http: &Client, app_id: &str, country: &str) -> Result<Vec<StoreOffer>, String> {
    let Some(key) = secrets::get(connection, "itad.api_key")? else { return Ok(Vec::new()); };
    let lookup = http
    ...
    if !lookup.status().is_success() { return Ok(Vec::new()); }
    let game: Value = lookup.json().map_err(|e| e.to_string())?;
    let Some(id) = game.get("id").and_then(Value::as_str) else { return Ok(Vec::new()); };
    let prices = http
        .post("https://api.isthereanydeal.com/games/prices/v3")
        .header("ITAD-API-Key", &key)
        .query(&[("country", country), ("deals", "false"), ("vouchers", "true")])
        .json(&vec![id])
        .send()
        .map_err(|e| format!("IsThereAnyDeal prices: {e}"))?;
    if !prices.status().is_success() { return Ok(Vec::new()); }
    let value: Value = prices.json().map_err(|e| e.to_string())?;
    let mut out = Vec::new();
    let deals = value.as_array().and_then(|a| a.first()).and_then(|v| v.get("deals")).and_then(Value::as_array);
    for deal in deals.into_iter().flatten() {
        let shop = deal.get("shop").and_then(|v| v.get("name")).and_then(Value::as_str).unwrap_or("Loja");
        let Some(price) = deal.get("price").and_then(|v| v.get("amount")).and_then(Value::as_f64) else { continue; };
        let currency = deal.get("price").and_then(|v| v.get("currency")).and_then(Value::as_str).unwrap_or("BRL");
        let regular = deal.get("regular").and_then(|v| v.get("amount")).and_then(Value::as_f64);
        let url = deal.get("url").and_then(Value::as_str).unwrap_or("");
        if url.is_empty() { continue; }
        out.push(StoreOffer {
            shop: shop.to_string(), kind: "official".into(), price, regular, currency: currency.to_string(),
            cut: deal.get("cut").and_then(Value::as_i64).unwrap_or(0), url: url.to_string(),
        });
    }
    Ok(out)
}

fn ggdeals_offers(connection: &Connection, http: &Client, app_id: &str) -> Result<(Vec<StoreOffer>, Option<String>), String> {
    let Some(key) = secrets::get(connection, "ggdeals.api_key")? else { return Ok((Vec::new(), None)); };
    let response = http
        .get("https://api.gg.deals/v1/prices/by-steam-app-id/")
        .query(&[("ids", app_id), ("key", key.as_str()), ("region", "br")])
        .send()
        .map_err(|e| format!("GG.deals: {e}"))?;
    if !response.status().is_success() { return Ok((Vec::new(), None)); }
    let value: Value = response.json().map_err(|e| e.to_string())?;
    if value.get("success").and_then(Value::as_bool) != Some(true) { return Ok((Vec::new(), None)); }
    let Some(game) = value.get("data").and_then(|v| v.get(app_id)) else { return Ok((Vec::new(), None)); };
    if game.is_null() { return Ok((Vec::new(), None)); }
    let url = game.get("url").and_then(Value::as_str).map(str::to_string);
    let prices = game.get("prices").unwrap_or(&Value::Null);
    let currency = prices.get("currency").and_then(Value::as_str).unwrap_or("BRL");
    let mut offers = Vec::new();
    if let Some(price) = prices.get("currentRetail").and_then(Value::as_f64) {
        offers.push(StoreOffer {
            shop: "Melhor loja oficial · GG.deals".into(), kind: "official".into(), price, regular: None,
            currency: currency.to_string(), cut: 0, url: url.clone().unwrap_or_else(|| format!("https://gg.deals/steam/app/{app_id}/")),
        });
    }
    if let Some(price) = prices.get("currentKeyshops").and_then(Value::as_f64) {
        offers.push(StoreOffer {
            shop: "Melhor keyshop · GG.deals".into(), kind: "keyshop".into(), price, regular: None,
            currency: currency.to_string(), cut: 0, url: url.clone().unwrap_or_else(|| format!("https://gg.deals/steam/app/{app_id}/")),
        });
    }
    Ok((offers, url))
...
pub fn compare(connection: &Connection, app_id: &str) -> Result<StoreComparison, String> {
    ...
    let country = crate::db::get_setting(connection, "store.country")?.unwrap_or_else(|| "BR".into());
    let mut offers = Vec::new();
    if let Some(offer) = steam { offers.push(offer); }
    offers.extend(itad_offers(connection, &http, app_id, &country)?);
    let (gg, gg_url) = ggdeals_offers(connection, &http, app_id)?;
    offers.extend(gg);
    let mut seen = HashSet::new();
    offers.retain(|o| seen.insert(format!("{}:{}:{:.2}:{}", o.shop.to_ascii_lowercase(), o.currency, o.price, o.url)));
