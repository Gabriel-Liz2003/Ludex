...
fn roots()->Vec<PathBuf>{
    let mut r=Vec::new(); for var in ["PROGRAMFILES","PROGRAMFILES(X86)"] { if let Ok(p)=std::env::var(var){ r.push(PathBuf::from(p)); } } r
}
pub fn scan()->Result<ProviderScan,String>{
    ...
    for root in roots(){ let Ok(entries)=fs::read_dir(&root) else{continue}; for e in entries.flatten(){let dir=e.path(); if !dir.is_dir()||!dir.join(".build.info").exists(){continue;} let title=e.file_name().to_string_lossy().to_string(); let id=title.to_ascii_lowercase().replace(' ',"-"); installations.push(ScannedInstallation{provider:"battlenet".into(),external_id:id,title,platform:"PC".into(),install_dir:Some(dir.to_string_lossy().to_string()),executable:None,installed:true,size_bytes:None,last_updated:None});}}
