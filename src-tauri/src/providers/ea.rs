...
fn roots() -> Vec<PathBuf> {
    ...
    if let Ok(p) = std::env::var("PROGRAMFILES") {
        r.push(PathBuf::from(p).join("EA Games"));
    }
    if let Ok(p) = std::env::var("PROGRAMFILES(X86)") {
        r.push(PathBuf::from(p).join("Origin Games"));
    }
    r
}
...
pub fn scan() -> Result<ProviderScan, String> {
    ...
    Ok(ProviderScan{id:"ea",name:"EA app",root,installations,can_launch:true,message:"Detecção local conservadora em EA Games/Origin Games; executável direto quando identificado com segurança.".into()})
}
