...
fn launcher_root() -> Option<PathBuf> {
    ...
    if let Ok(p) = std::env::var("PROGRAMFILES(X86)") { roots.push(PathBuf::from(p).join("Epic Games").join("Launcher")); }
    if let Ok(p) = std::env::var("PROGRAMFILES") { roots.push(PathBuf::from(p).join("Epic Games").join("Launcher")); }
...
fn parse_manifest(content: &str) -> Result<Option<ScannedInstallation>, String> {
    ...
    let app = m.app_name.filter(|s| !s.trim().is_empty()).ok_or_else(|| "AppName ausente".to_string())?;
    let title = m.display_name.unwrap_or_else(|| app.clone());
    let install = m.install_location.filter(|s| !s.trim().is_empty());
    let executable = match (&install, m.launch_executable.filter(|s| !s.trim().is_empty())) {
    ...
    Ok(Some(ScannedInstallation {
        provider: "epic".into(), external_id, title, platform: "PC".into(),
        install_dir: install.clone(), executable,
        installed: !m.b_is_incomplete_install && install.as_deref().is_some_and(|p| Path::new(p).exists()),
...
pub fn scan() -> Result<ProviderScan, String> {
    ...
    if let Some(path) = dir.as_ref().filter(|p| p.exists()) {
        for entry in fs::read_dir(path).map_err(|e| e.to_string())?.flatten() {
            let p = entry.path();
            if p.extension().and_then(|s| s.to_str()).is_some_and(|s| s.eq_ignore_ascii_case("item")) {
                if let Ok(content) = fs::read_to_string(&p) {
                    if let Ok(Some(item)) = parse_manifest(&content) { installations.push(item); }
                }
            }
        }
    }
    let root = launcher_root().or(dir.clone());
    Ok(ProviderScan { id:"epic", name:"Epic Games Store", root, installations, can_launch:true,
